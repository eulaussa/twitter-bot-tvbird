    ...
    for mention in mentions:
        if not findMentionID(mention.id):
            ...
            if mention.full_text == '@tvbirdtweets':
                ...
                tweet = ' Dünün birincisi ' + str(df.at[1,'RTG']) + ' reyting ve ' + str(df.at[1,'SHARE']) 
                        + ' share ile ' + df.at[1, 'BAŞLIK'].title()
                ...
                tweet = ' Bugün en yüksek reyting ortalamasıyla izlenen program türü ' 
                        ...
                        + ' olmuş. Onları takiben ' + str(round(df_RTG.at[1, 'RTG'],2)) +  ' ile '
                        + beautifyWords(df_RTG.at[1, 'TÜR']) + ' ve ' + str(round(df_RTG.at[2, 'RTG'],2)) + ' ile ' 
                        + beautifyWords(df_RTG.at[2, 'TÜR']) + ' geliyor.'
                api.update_status('@' + mention.user.screen_name + tweet, mention.id)
